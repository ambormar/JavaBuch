...
public class SubKlasse_KreisSegment extends SuperKlasse_Kreis {
	
	double winkel;	
	
	// initialisierungs default konstruktor anstelle von automatischem default konstruktor
	SubKlasse_KreisSegment(){		 
		super();		// aufrufen des konstruktors der superklasse	
		winkel = 90;	// winkel auf 90°
	}
	// custom konstruktor mit zugriff auf konstruktor der superklasse 
	SubKlasse_KreisSegment(double r, double w) {
		super(r);		// aufrufen des konstruktors der superklasse
		winkel = w;
	}
	// neue angepasste methoden für fläche und umfang damit werte bei kreissegment stimmen
	// die anderen methoden kann man lassen, weil sie immer noch stimmen
	// super. für getFlaeche, damit klar auf die methode der superklasse zugegriffen & nicht die methode sich selber aufruft
	public double getFlaeche(){
		return super.getFlaeche() * winkel / 360; //  mit super auf fläche Kreis zugreifen, dann mathematisch auf segment umrechnen 
	}
	
	public double getUmfang(){    // name der methode muss nicht mit methodenname der superklasse übereinstimmen, ist aber simpler
		return super.getUmfang() * winkel / 360 + 2 * getRadius(); // super bei getRadius nicht nötig, weil methode nur in SuperKlasse_Kreis & public
	}
}
